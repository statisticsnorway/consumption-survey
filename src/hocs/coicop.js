...
const SURVEY_SERVICE_HOST =
  process.env.NEXT_PUBLIC_SURVEY_SERVICE_HOST || 'http://localhost:5555'
const COICOP_SEARCH_ENDPOINT = `${SURVEY_SERVICE_HOST}/v1/searchterm`
...
export const useCoicopSearch = (term) => {
  ...
  const searchUrl = `${COICOP_SEARCH_ENDPOINT}?text=${coicopSearchTerm}`
  const [{ data, loading, error }, refetch] = useAxios(searchUrl, {
    useCache: false,
  })
